...
export class FormService<T extends { [K in keyof T]: AbstractControl }, UserTypes> implements OnDestroy {
  ...
  public addControl<R>(
    ...
    formArray: FormArray<FormControl<R | null>>,
    ...
    const control: FormControl<R | null> = formControl(initialValue, initialDisabledState, initialValidators);
    formArray.push(control);
    return control;
  }


  public addGroup<R extends { [K in keyof R]: AbstractControl }>(
    formGroup: (initialValue?: R | null, initialDisabledState?: boolean | null, initialValidators?: ValidatorFunctions) => FormGroup<R>,
    formArray: FormArray<FormGroup<R>>,
    initialValue?: R | null | undefined,
    initialDisabledState?: boolean | null,
    initialValidators?: ValidatorFunctions
  ): FormGroup<R> {
    markAsUiChange(formArray);
    formGroup = formGroup.bind(this);
    const control: FormGroup<R> = formGroup(initialValue, initialDisabledState, initialValidators);
    formArray.push(control);
    return control;
  }


  public removeControl<R>(
    formArray: FormArray<FormControl<R | null>>,
    formControl: FormControl<R | null>
  ): void {
    markAsUiChange(formArray);
    formArray.removeAt(formArray.controls.indexOf(formControl));
  }


  public removeGroup<R extends { [K in keyof R]: AbstractControl }>(
    formArray: FormArray<FormGroup<R>>,
    formControl: FormGroup<R>
  ): void {
    markAsUiChange(formArray);
    formArray.removeAt(formArray.controls.indexOf(formControl));
  }
