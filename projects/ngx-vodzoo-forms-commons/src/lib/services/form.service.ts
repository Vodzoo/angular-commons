...
export class FormService<T extends { [K in keyof T]: AbstractControl }, UserConfig, UserTypes> implements OnDestroy {
   ...
   * Example:
   * ```ts
   * protected override formFieldsLogic(): FormControlsLogic<PersonBase, any, Date> {
   *     return {
   *       address: this.formAddressService.mergeLogicWith({
   *         country: (spec) => {
   *           this.formAddressService.getFormFieldsLogic().country?.(spec); // run all logic from formAddressService
   *
   *           const onInitFn: FormFieldLogicFnSpecArgsFn<Address, any, Date> = ({form, index, config}) => { // INIT LOGIC }
   *
   *           const onValueChangeFn: FormFieldLogicFnSpecArgsFn<Address, any, Date> = ({form, index, config}): FormFieldLogicFnState | void => {
   ...
   *           this.formAddressService.getFormFieldsLogic().voivodeship?.({onValueChange, onConfigChange, onLogicRecalculate}); // no onInit and onDestroy logic will be triggered from formAddressService
   *           onInit?.(({form}) => console.log(form));
   *           this.formAddressService.getFormFieldsLogic().voivodeship?.({onInit}); // only onInit logic will be triggered from formAddressService
   *         },
   *       }),
   *     };
   *   }
