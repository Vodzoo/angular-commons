...
export class FormConfigDirective<T extends { [K in keyof T]: AbstractControl }, UserConfig, UserTypes> implements OnInit {
   ...
   * Signals
   */
  private formValueChanges: Signal<FormValue<T, UserTypes> | undefined> = signal(undefined);
  ...
  private runLogicEffect = effect(() => {
    this.runLogic(this.controlsLogic(), this.controlsConfigChange());
  });
  ...
  private runLogic(logic: FormControlsLogic<T, UserConfig, UserTypes>, config: FormControlsConfigChange<T, UserConfig, UserTypes>): void {
    Object.keys(logic).forEach((key: string) => {
      ...
      if (typeof fieldLogic === 'function') {
        fieldLogic(this.formDirective.form, config, this.formDirective.formIndex);
      }
    })
  }
}
...
export type FormFieldLogicFn<T extends { [K in keyof T]: AbstractControl }, UserConfig, UserTypes> = (form: FormGroup<T>, config: FormControlsConfigChange<T, UserConfig, UserTypes>, index?: number) => void;
